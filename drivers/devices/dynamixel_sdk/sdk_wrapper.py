...
class DynamixelMotor(object):
    ...
    def set_dxl_pro_vel(self, dxl_id, profile_velocity):
        # if PortHandler.openPort() and PortHandler.setBaudRate(self.baud_rate):
        #     print("Port opened and baudrate set.")
        #     _, _ = self._packet_handler.write4ByteTxRx(PortHandler, dxl_id, self._control_table.ADDR_PRO_VELOCITY, profile_velocity)
        #     _, _ = self._packet_handler.write4ByteTxRx(PortHandler, dxl_id, self._control_table.ADDR_PRO_ACCELERATION, profile_acceleration)
        #     _, _ = self._packet_handler.write4ByteTxRx(PortHandler, dxl_id, self._control_table.ADDR_VELOCITY_LIMIT, velocity_limit)
        #     print("Settings applied: Profile Velocity, Profile Acceleration, Velocity Limit.")
        # else:
        ...
        dxl_comm_result, dxl_error = self._packet_handler.write4ByteTxRx(port=self._port_handler,
                                                                         dxl_id=dxl_id,
                                                                         address=self._control_table.ADDR_PRO_VELOCITY,
    ...
    def set_dxl_goal_pos_vel_sync(self, tgt_pos_list: list or tuple, tgt_vel_list: list or tuple, dxl_id_list: list or tuple) -> bool:
        if self._group_sync_write is None:
            raise Exception(
                "The group sync write is not enabled. Please set toggle_group_sync_write to True when initializing the class.")
        assert isinstance(tgt_pos_list, (list, tuple)) and isinstance(dxl_id_list, (
        ...
        assert len(tgt_pos_list) == len(
            dxl_id_list), "The length of the target position list and the motor id list should be the same."
        assert isinstance(tgt_vel_list, (list, tuple)) and isinstance(dxl_id_list, (
            list, tuple)), "The input should be a list or tuple."
        assert len(tgt_vel_list) == len(
            dxl_id_list), "The length of the target vel list and the motor id list should be the same."

        for tgt_pos in tgt_pos_list:
            assert isinstance(tgt_pos, int), "The target position should be an integer."
            assert self._control_table.DXL_MIN_POSITION_VAL <= tgt_pos <= self._control_table.DXL_MAX_POSITION_VAL, "The target position is out of range."

        for tgt_vel in tgt_vel_list:
            assert isinstance(tgt_vel, int), "The target position should be an integer."
            assert self._control_table.DXL_MIN_VELOCITY_VAL <= tgt_vel <= self._control_table.DXL_MAX_VELOCITY_VAL, "The target position is out of range."
            # DXL_MIN_VELOCITY_VAL = 0,
            # DXL_MAX_VELOCITY_VAL = 1023,

        for i, dxl_id in enumerate(dxl_id_list):
            assert isinstance(dxl_id, int), "The motor id should be an integer."
            param_goal_position = [int(tgt_pos_list[i] % 256),
                                   int(tgt_pos_list[i] >> 8),
                                   int(tgt_vel_list[i] % 256),
                                   int(tgt_vel_list[i] >> 8)
                                       ]
...
if __name__ == "__main__":
    ...
    com = 'COM4'
    dxl_con = DynamixelMotor(com, baud_rate=peripheral_baud, toggle_group_sync_write=True)
    # r = dxl_con.ping()
    ...
    # dxl_con.set_dxl_goal_pos_sync([1000, 2500], [0, 1])
    tgt_1 = dxl_con.get_dxl_pos(1)
    tge_0 = dxl_con.get_dxl_pos(0)
    dxl_con.set_dxl_pro_vel(0, 20000)
    dxl_con.set_dxl_pro_vel(1, 0)
    while 1:
        dxl_con.set_dxl_goal_pos_sync([tge_0, tgt_1], [0, 1])
        time.sleep(0.05)
        # tgt_1 += 100
        # tge_0 += 100
        tgt_1 += -100
    ...
    # dxl_con.set_dxl_goal_pos_vel_sync([0, 0], [500,500], [0,1])
    time.sleep(100)
    ...
    while 1:
        ...
        dxl_con.set_dxl_position_p_gain(50, 0)
        dxl_con.set_dxl_goal_current(80, 1)
        dxl_con.set_dxl_goal_current(80, 0)
        dxl_con.set_dxl_goal_pos(s1, 1)
        dxl_con.set_dxl_goal_pos(s2, 0)
        print(dxl_con.get_dxl_current(1), dxl_con.get_dxl_current(0))
        # exit(0)
        input('b')
        ...
        dxl_con.set_dxl_goal_current(170, 0)
        dxl_con.set_dxl_position_p_gain(20, 1)
        dxl_con.set_dxl_position_p_gain(20, 0)
        dxl_con.set_dxl_goal_pos(s1, 1)
        dxl_con.set_dxl_goal_pos(s2, 0)
