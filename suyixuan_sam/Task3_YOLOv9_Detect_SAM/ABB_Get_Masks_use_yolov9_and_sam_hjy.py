...
# SAM 模型加载路径
sam_checkpoint = r"/home/suyixuan/AI/Pose_Estimation/WRS_FoundationPose_YOLOv9_SAM/weights/sam_vit_h_4b8939.pth"
model_type = "vit_h"

# 结果保存路径
save_folder = '/home/suyixuan/AI/Pose_Estimation/WRS_FoundationPose_YOLOv9_SAM/suyixuan_sam/Task3_YOLOv9_Detect_SAM/results_yolodetect_API'
save_mask_folder = '/home/suyixuan/AI/Pose_Estimation/WRS_FoundationPose_YOLOv9_SAM/demo_data/514'
...
# ----------------- 保存目标掩码图像（白色物体，黑色背景）-----------------
def save_object_mask_on_black_background(img, masks, bboxes, labels, save_mask_folder, target_labels):
    """
    ...
    # 创建 mask 子文件夹
    mask_subfolder = os.path.join(save_mask_folder, 'masks')
    os.makedirs(mask_subfolder, exist_ok=True)
    ...
    for i, (mask, bbox, label) in enumerate(zip(masks, bboxes, labels)):
        ...
        if label.split()[0] in target_labels:
            # 保存掩码图像
            save_masks_path = os.path.join(save_mask_folder, f"00000{i + 1}.png")  # 使用物体名称作为文件名
            cv2.imwrite(save_masks_path, mask_img)
            print(f"物体 {label} 的掩码图像已保存: {save_masks_path}")

            # # 保存到 mask 子文件夹
            # save_masks_path_mask_folder = os.path.join(mask_subfolder, f"00000{i + 1}.png")
            # cv2.imwrite(save_masks_path_mask_folder, mask_img)
            # print(f"物体 {label} 的掩码图像已保存到 mask 子文件夹: {save_masks_path_mask_folder}")


# ----------------- 保存带有 YOLO 检测结果的图像 -----------------
def save_yolo_detection_image(img, detections, save_path, detect_api, color=(0, 255, 0), thickness=2):
    """
...
# ----------------- 主函数 -----------------
def main(source, weights, target_labels):  # 修改 main 函数的参数
    # 初始化 YOLOv9 API
    weights = weights  # YOLOv9 权重文件路径
    ...
    # 保存目标掩码图像
    save_object_mask_on_black_background(img, masks, bboxes, labels, save_mask_folder, target_labels)

    # 保存带有 YOLO 检测结果的图像
    save_yolo_detection_image(img, detections, yolo_detection_image_path, detect_api)
...
if __name__ == "__main__":
    # 定义参数
    source = '/home/suyixuan/AI/Pose_Estimation/WRS_FoundationPose_YOLOv9_SAM/demo_data/514/rgb/000001.png'  # 图像路径
    weights = '/home/suyixuan/AI/Pose_Estimation/WRS_FoundationPose_YOLOv9_SAM/weights/osgj.pt'  # YOLOv9 权重文件路径
    target_labels = "j"

    # 调用 main 函数，传递参数
    main(source, weights, target_labels)
