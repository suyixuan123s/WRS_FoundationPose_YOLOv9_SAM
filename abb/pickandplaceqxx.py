...
import robot_sim.robots.gofa5.gofa52 as gf5  ## gofowithag 将爪子加入了cc
...
if __name__ == '__main__':
    ...
    obj_list.append(ball1)
    obj_list.append(ball2)
    obj_list.append(ball3)
    obj_list.append(ball4)
    ...
    for init_pos in init_posobj:
        init_pos[2] = 0.1
    print("kk,", init_posobj)
    obj0 = cm.gen_box(extent=[.08, 0.1, 0.15], rgba=[0, 0, 1, 1])
    obj0.set_pos(pos=np.array(init_posobj[58]))
    print("obj0pos", obj0.get_pos())
    obj0.attach_to(base)
    gm.gen_frame(pos=obj0.get_pos(), rotmat=obj0.get_rotmat()).attach_to(base)

    obj1 = cm.gen_box(extent=[0.03, 0.03, 0.08], rgba=[1, 0, 0, 1])
    obj1.set_pos(pos=np.array([.6, -0.3, 0.06]))
    obj1.attach_to(base)

    obj2 = cm.gen_box(extent=[0.03, 0.03, 0.03], rgba=[0, 1, 0, 1])
    obj2.set_pos(np.array(init_posobj[105]))
    obj2.attach_to(base)

    obstacle_list.append(obj0)
    obstacle_list.append(obj2)
    ...
    grasp_info_list = gpa.load_pickle_file(objcm_name="box1", root=None, file_name="ag145_qxx_grasp478.pickle")
    print(len(grasp_info_list))  ##---box0  274
    # base.run()
    start_pos = np.array(obj1.get_pos())
    start_rotmat = np.array(obj1.get_rotmat())
    start_homo = rm.homomat_from_posrot(start_pos, start_rotmat)

    # gm.gen_frame(start_pos,start_rotmat).attach_to(base)##   末端执行器和物体是不一样的

    end_pos = np.array(obj00.get_pos())
    end_rotmat = np.array(obj00.get_rotmat())
