# rrt_star_connect.py 实现了 RRT Connect* 算法,结合了 RRT* 和 RRT-Connect 的特点
# 算法通过从起点和目标同时扩展树(双向扩展)并尝试连接两棵树来加速路径搜索.每次扩展节点时进行 路径优化 和 碰撞检测
# 确保路径最短且无碰撞.最终通过 路径平滑 技术优化路径,确保得到更短、平滑的路径
