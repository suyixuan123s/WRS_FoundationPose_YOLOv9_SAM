...
def show_multi_ik(obj_rotmat, obj_pos, grasp):
    ...
    hndfa.fix_to(pos=hnd_pos,
                 rotmat=hnd_rotmat, jawwidth=.04)
    for _ in range(100):
        seed = np.asarray([random.uniform(-np.pi,np.pi), random.uniform(-np.pi,np.pi), random.uniform(-np.pi,np.pi), random.uniform(-np.pi,np.pi),random.uniform(-np.pi,np.pi),random.uniform(-np.pi,np.pi)])
        # hndfa.gen_meshmodel(rgba=(1, 0, 0, 0.05)).attach_to(base)
        jnt_values = robot.ik(component_name=component_name,
                              tgt_pos=hnd_pos,
                              tgt_rotmat=hnd_rotmat,
...
if __name__ == '__main__':
    ...
    object_start_rotmat = rm.rotmat_from_axangle((0,0,1), np.radians(180)).dot(rm.rotmat_from_axangle((1,0,0), np.radians(-90)).dot(rm.rotmat_from_axangle((0,0,1),np.radians(180))))
