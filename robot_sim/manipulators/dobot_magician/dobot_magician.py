...
class DobotMagician(mi.ManipulatorInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), homeconf=np.zeros(3), name='dobot_magician', enable_cc=True):
    ...
    def _mimic_jnt_values(self, jnt_values):
        ...
        总是将第四个关节(j4)的值设置为第二个关节(j2)和第三个关节(j3)的和
        :param jnt_values: 输入的关节值数组.
        :return: 返回处理后的新关节值数组.
        author: weiwei
        date: 20220214
        """
