...
class Lite6WRSGripper2(gp.GripperInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), cdmesh_type='convex_hull', name='lite6_wrs_gripper2',
        ...
        # gripper base
        self.lft = jl.JLChain(pos=cpl_end_pos, rotmat=cpl_end_rotmat, homeconf=np.zeros(1), name='base')
        self.lft.jnts[1]['loc_pos'] = np.array([0, 0, 0])
        self.lft.lnks[0]['name'] = "base"
        self.lft.lnks[0]['loc_pos'] = np.zeros(3)
        self.lft.lnks[0]['collision_model'] = cm.CollisionModel(os.path.join(this_dir, "meshes", "base_v2.stl"),
                                                                 expand_radius=.001)
        self.lft.lnks[0]['rgba'] = [.57, .57, .57, 1]

        self.lft.lnks[1]['name'] = "realsense_dual"
        self.lft.lnks[1]['loc_pos'] = np.zeros(3)
        self.lft.lnks[1]['collision_model'] = cm.CollisionModel(os.path.join(this_dir, "meshes", "dual_realsense.stl"),
                                                                 expand_radius=.001)
        self.lft.lnks[1]['rgba'] = [.37, .37, .37, 1]

        self.lft.jnts[3]['loc_pos'] = np.array([0.01492498, 0.005, .05])
        self.lft.jnts[3]['loc_rotmat'] = rm.rotmat_from_euler(0, 0, -math.pi)
        self.lft.jnts[3]['type'] = 'prismatic'
        self.lft.jnts[3]['motion_rng'] = [.0, .025]
        self.lft.jnts[3]['loc_motionax'] = np.array([0, -1, 0])
        ...
        self.rgt.jnts[1]['type'] = 'prismatic'  # 滑动关节
