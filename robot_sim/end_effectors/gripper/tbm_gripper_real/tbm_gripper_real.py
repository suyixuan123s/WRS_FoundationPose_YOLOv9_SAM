...
class TBMGripper(gp.GripperInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), cdmesh_type='', name='tbm_gripper', enable_cc=True):
        super().__init__(pos=pos, rotmat=rotmat, cdmesh_type=cdmesh_type, name=name)
        this_dir, this_filename = os.path.split(__file__)
        cpl_end_pos = self.coupling.jnts[-1]['gl_posq']
        cpl_end_rotmat = self.coupling.jnts[-1]['gl_rotmatq']
        # left finger
        self.lft_fgr = jl.JLChain(pos=cpl_end_pos,
        ...
        # right finger
        # self.rgt_fgr = jl.JLChain(pos=cpl_end_pos, rotmat=cpl_end_rotmat, homeconf=np.zeros(1), name='lft_outer',cdprimitive_type='cylinder')
        ...
        self.lft_fgr.lnks[0]['mesh_file'] = cm.gen_box(extent=[0.3, 0.2, 0.1])
