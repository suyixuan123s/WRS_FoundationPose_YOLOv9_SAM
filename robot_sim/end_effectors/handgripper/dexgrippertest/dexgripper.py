...
class DexGripper(gf.DexGripperInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="ur5e_conveyorbelt", enable_cc=True):
        ...
        self.base_stand.lnks[0]['collision_model'] = cm.CollisionModel(
            ...
            cdprimit_type="box", expand_radius=.005, userdefined_cdprimitive_fn=None)
        self.base_stand.jnts[0]['loc_pos'] = np.array([0, 0, 0])
        ...
        self.finger_lft = lf.Leftfinger(pos=self.base_stand.jnts[0]['gl_posq'],
                            rotmat=self.base_stand.jnts[0]['gl_rotmatq'],
                            homeconf=arm_homeconf,
                            name='arm', enable_cc=False)
        # gripper
        self.finger_rgt = lf.Leftfinger(pos=self.base_stand.jnts[0]['gl_posq'],
                                        rotmat=np.dot(rm.rotmat_from_axangle(np.array([0,0,1]), np.deg2rad(90)),self.base_stand.jnts[-1]['gl_rotmatq']),
                                        homeconf=arm_homeconf,
                                        name='arm', enable_cc=False)
        # tool center point
        ...
        if enable_cc:
            self.enable_cc()
        # component map
        self.finger_dict['lft'] = self.finger_lft
        self.finger_dict['rgt'] = self.finger_rgt
    ...
    def enable_cc(self):
        ...
        self.cc.add_cdlnks(self.finger_lft, [0, 1, 2, 3])
        self.cc.add_cdlnks(self.finger_rgt, [0, 1, 2, 3])
        ...
        activelist = [self.base_stand.lnks[0],
                      self.finger_lft.lnks[1],
                      self.finger_lft.lnks[2],
                      self.finger_lft.lnks[3]]
    ...
    def gen_stickmodel(self,
        ...
        self.finger_lft.gen_stickmodel(tcp_jnt_id=tcp_jnt_id,
                                tcp_loc_pos=tcp_loc_pos,
                                tcp_loc_rotmat=tcp_loc_rotmat,
                                toggle_tcpcs=toggle_tcpcs,
                                toggle_jntscs=toggle_jntscs,
                                toggle_connjnt=toggle_connjnt).attach_to(stickmodel)
        self.finger_rgt.gen_stickmodel(toggle_tcpcs=False,
    ...
    def gen_meshmodel(self,
        ...
        self.finger_lft.gen_meshmodel(tcp_jnt_id=tcp_jnt_id,
                               tcp_loc_pos=tcp_loc_pos,
                               tcp_loc_rotmat=tcp_loc_rotmat,
                               toggle_tcpcs=toggle_tcpcs,
                               toggle_jntscs=toggle_jntscs,
                               rgba=rgba).attach_to(meshmodel)
        self.finger_rgt.gen_meshmodel(toggle_tcpcs=False,
                               toggle_jntscs=toggle_jntscs,
                               rgba=rgba).attach_to(meshmodel)
        for obj_info in self.oih_infos:
            objcm = obj_info['collision_model']
...
if __name__ == '__main__':
    ...
    robot_s = DexGripper(enable_cc=True)
