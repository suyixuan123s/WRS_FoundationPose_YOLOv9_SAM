...
class Handgripper(gf.DexGripperInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name='ur3edual', enable_cc=True):
        ...
        # self.lft_body.jnts[1]['loc_pos'] = np.array([0, 0, 0])  # left from robot_s view
        # self.lft_body.jnts[1]['loc_rotmat'] = rm.rotmat_from_euler(math.pi / 2.0, 0,math.pi / 2.0)  # left from robot_s view
        self.base.lnks[0]['name'] = "ur3e_dual_base"
        ...
        # self.manipulator_dict['rgt_hnd'] = self.rgtfinger  # specify which hand is a gripper installed to
        # self.manipulator_dict['lft_hnd'] = self.lftfinger  # specify which hand is a gripper installed to
