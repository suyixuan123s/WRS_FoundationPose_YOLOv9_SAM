...
class CobottaPipette(ri.RobotInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="cobotta_pipette", enable_cc=True):
        ...
        # gripper
        self.gripper_loc_rotmat = rm.rotmat_from_axangle([0, 0, 1],
                                                         np.pi)  # 20220607 rotate the pipetting end with 180^o.
...
if __name__ == '__main__':
    ...
    robot_s = CobottaPipette(enable_cc=True)
    robot_s.jaw_to(.02)
