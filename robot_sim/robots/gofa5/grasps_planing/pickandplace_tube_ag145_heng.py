...
if __name__ == '__main__':
    ...
    rack_10ml.set_pos(np.array([.6, -.3, -0.015]))
    ...
    tube1.set_pos(np.array([.555, -.3, 0.113 - 0.015]))
    tube1.set_rotmat()
    tube1.attach_to(base)
    obstacle_list.append(tube1)

    tube6 = cm.CollisionModel(f"objects/{tube_name}.stl")
    tube6.set_rgba([.0, 1, .0, 1])
    tube6.set_pos(np.array([.555, -.3 + 0.025, 0.113 - 0.015]))
    tube6.set_rotmat()
    tube6.attach_to(base)
    obstacle_list.append(tube6)

    tube7 = cm.CollisionModel(f"objects/{tube_name}.stl")
    tube7.set_rgba([.0, 1, .0, 1])
    tube7.set_pos(np.array([.555, -.3 - 0.025, 0.113 - 0.015]))
    tube7.set_rotmat()
    tube7.attach_to(base)
    # obstacle_list.append(tube7)


    tube2 = cm.CollisionModel(f"objects/{tube_name}.stl")
    tube2.set_rgba([.0, 1, .0, 1])
    tube2.set_pos(np.array([.585, -.3, 0.113 - 0.015]))
    tube2.set_rotmat()
    ...
    tube3.set_pos(np.array([.615, -.3, 0.113 - 0.015]))
    tube3.set_rotmat()
    ...
    # tube4.set_pos(np.array([.645, -.3, 0.113 - 0.015]))
    # tube4.set_rotmat()
    # tube4.attach_to(base)
    # obstacle_list.append(tube4)

    # tube5 = cm.CollisionModel(f"objects/{tube_name}.stl")
    # tube5.set_rgba([.0, 1, .0, 1])
    # tube5.set_pos(np.array([.645, -.275, 0.113-0.015]))
    # tube5.set_rotmat()
    ...
    # obj.set_pos(np.array([.615, -.3, 0.113-0.015]))
    # obj.set_rotmat()
    # obj.attach_to(base)
    #
    # # object_goal
    ...
    # obj_goal.set_pos(np.array([.645, -.3, 0.113]))
    # obj_goal.set_rotmat()
    ...
    start_pos = tube7.get_pos()
    start_rotmat = tube7.get_rotmat()
    goal_pos = np.array([.645, -.275, 0.113 - 0.015])
