...
class GOFA5(ri.RobotInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="gofa5", enable_cc=True):
        ...
        # base plate
        self.base_stand = jl.JLChain(pos=pos - [0.13747, -0.03748, 0.015],
                                     rotmat=rotmat,
                                     homeconf=np.zeros(0),
                                     name='base_stand')
        ...
        self.base_stand2 = jl.JLChain(pos=self.base_stand.jnts[-1]['gl_posq'],
    ...
    def fix_to(self, pos, rotmat):
        ...
        self.base_stand2.fix_to(pos=self.base_stand.jnts[-1]['gl_posq'], rotmat=self.base_stand.jnts[-1]['gl_rotmatq'])
    ...
    def get_jnt_values(self, component_name):
        ...
        if component_name in self.manipulator_dict:
            ...
            raise ValueError("给定的组件名称不受支持! The given component name is not supported!")
    ...
    def ik(self,
           ...
           toggle_debug=False):
        """
        执行逆运动学 (IK) 计算,求解机械臂或手部达到目标位置和姿态所需的关节角度

        :param component_name: 要进行 IK 计算的组件名称 ("arm" 表示机械臂, "hnd" 表示手部).默认为 "arm"
        :param tgt_pos: 目标位置 (3D 向量).默认为原点
        :param tgt_rotmat: 目标旋转矩阵 (3x3 矩阵).默认为单位矩阵
        :param seed_jnt_values: 关节角度的种子值,用于引导 IK 求解器.如果为 None,则使用默认种子值
        :param max_niter: IK 求解器的最大迭代次数.默认为 200
        :param num_seeds: 用于IK求解的种子数量,默认为2
        :param tcp_jnt_id: 工具中心点 (TCP) 所在的关节 ID
        :param tcp_loc_pos: TCP 相对于 `tcp_jnt_id` 关节的局部位置
        :param tcp_loc_rotmat: TCP 相对于 `tcp_jnt_id` 关节的局部旋转矩阵
        :param local_minima: 指定如何处理局部最小值.默认为 "end"
        ...
        if component_name == "arm" or component_name == "hnd":
            ...
            tgt_pos = tgt_pos - np.dot(tgt_rotmat, self.hnd_dict[component_name].jaw_center_pos)
