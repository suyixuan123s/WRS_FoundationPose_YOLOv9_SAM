...
import robot_sim.manipulators.gofa10_arm.gofa10_arm as rbt
...
class GOFA10(ri.RobotInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="gofa10_arm", enable_cc=True):
        ...
        # machine
        self.machine = machine.MachineTool(pos=pos + np.array([0, -0.7, -0.82]),
                                           rotmat=rm.rotmat_from_axangle(np.array([0, 0, 1]), np.pi), name='machine',
                                           enable_cc=False)
