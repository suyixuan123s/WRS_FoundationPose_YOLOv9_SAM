...
class UR5EConveyorBelt(ri.RobotInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name="ur5e_conveyorbelt", enable_cc=True):
        ...
        self.base_plate = jl.JLChain(pos=pos,
                                     ...
                                     homeconf=np.zeros(3),
                                     name='base_plate')
        self.base_plate.jnts[1]['loc_pos'] = np.array([.9, -1.5, -0.06])
        self.base_plate.jnts[2]['loc_pos'] = np.array([0, 1.23, 0])
        self.base_plate.jnts[3]['loc_pos'] = np.array([0, 0, 0])
        self.base_plate.jnts[4]['loc_pos'] = np.array([-.9, .27, 0.06])
        self.base_plate.lnks[0]['collision_model'] = cm.CollisionModel(
        ...
        self.base_plate.lnks[1]['mesh_file'] = os.path.join(this_dir, "meshes", "conveyor.stl")
        self.base_plate.lnks[1]['rgba'] = [.35, .45, .35, 1]

        self.base_plate.lnks[2]['mesh_file'] = os.path.join(this_dir, "meshes", "camera_stand.stl")
        self.base_plate.lnks[2]['rgba'] = [.55, .55, .55, 1]

        self.base_plate.lnks[3]['mesh_file'] = os.path.join(this_dir, "meshes", "cameras.stl")
        self.base_plate.lnks[3]['rgba'] = [.55, .55, .55, 1]
        ...
        self.arm = rbt.UR5E(pos=self.base_plate.jnts[-1]['gl_posq'],
    ...
    def enable_cc(self):
        ...
        super().enable_cc()
        self.cc.add_cdlnks(self.base_plate, [0, 1, 2, 3])
        ...
        activelist = [self.base_plate.lnks[0],
                      self.base_plate.lnks[1],
                      self.base_plate.lnks[2],
                      self.base_plate.lnks[3],
                      self.arm.lnks[1],
                      self.arm.lnks[2],
                      self.arm.lnks[3],
                      self.arm.lnks[4],
