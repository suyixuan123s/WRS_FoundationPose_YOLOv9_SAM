...
def expand_adj(vertices, faces, face_normals, seed_face_id, rgt_ele_mat, sel_mat, max_normal_bias_angle):
    """
    找到一个面片的邻接面片,新增面片的法线应与种子面片的法线一致
    这是一个迭代函数

    :param vertices: 顶点数组
    :param faces: 面片数组
    :param face_normals: 面片法线数组
    :param seed_face_id: 要扩展的面片索引
    :param rgt_ele_mat: 右元素矩阵
    :param sel_mat: 选择矩阵
    :param max_normal_bias_angle: 最大法线偏差角度
    :return: 面片 ID 列表
    ...
    # 找出与seed_face_id相连的所有角度受限的 seed_face_id
...
def over_segmentation(objcm, max_normal_bias_angle=rm.math.pi / 12, toggle_face_id_pair_for_curvature=False):
    """
    使用网格过度分割计算簇

    :param objcm: 对象网格
    :param max_normal_bias_angle: 相邻面片之间的最大法线偏差角度
    :param toggle_face_id_pair_for_curvature: 是否返回用于曲率计算的面片 ID 对
    :return: 分割结果     TODO replace np.arccos with math.cos
